...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Chamar backend Go para atualizar dados do usuário por ID
    const response = await fetch(`${BACKEND_API_URL}/user/${encodeURIComponent(userId)}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      return NextResponse.json({ success: false, error: data?.message || data?.error || 'Failed to update user' }, { status: response.status })
    }
